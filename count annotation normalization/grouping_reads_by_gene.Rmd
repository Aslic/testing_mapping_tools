...
star_genom<-read.csv("genome_al_star_counts.csv",header = TRUE, stringsAsFactors = FALSE)
...
dim(salmon_1)
star_1<-star_genom[rowSums(star_genom[,2:ncol(star_genom)])>16,]
dim(star_1)
star_2<-star_genom_gene[rowSums(star_genom_gene[,2:ncol(star_genom_gene)])>16,]
dim(star_2)
```
Star has given us the lowest number of transcripts. However within star alignment grouping the features (exons) by gene vs by transcrip gives us higher number. To be able to make results comparable let's convert everthing to gene name.
```{r}
...
```{r, echo=FALSE}
star_lookup1<-subset(refseq_lookup, select=c(Gene.name, Transcript.stable.ID))
star_lookup2<-subset(refseq_lookup, select=c(Gene.name, Gene.stable.ID))
names(star_lookup1)[2]<-"id"
names(star_lookup2)[2]<-"id"
star_1_m<-join(star_1, star_lookup1,by="id")
star_2_m<-join(star_2, star_lookup2, by="id")
star_1_m<-star_1_m[!duplicated(star_1_m),]
...
gene_star1<-read_per_gene(star_1_m)
...
write.csv(gene_star1,"gene_star1",row.names = FALSE)
